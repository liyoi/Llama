...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.pos_embed_method == 'rope':
            xq, xk = apply_rotary_emb(key, query, self.freqs_cis.to(key.device))
            query, key = xq, xk
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, num_heads, head_size, block_size, n_embedding=360, pos_embed_method="rope"):
        super().__init__()
        # 多头注意力由多个注意力叠加
        self.heads = nn.ModuleList(
            [Head(head_size, block_size=block_size, n_embedding=n_embedding, pos_embed_method=pos_embed_method) for _ in
             range(num_heads)])
...
#     freqs_cis = None
#
#     def __init__(self, head_size, n_embedding=360, block_size=16, dropout=0.2, pos_embed_method="rope"):
#         super().__init__()
#         self.key = nn.Linear(n_embedding, head_size, bias=False)
#         self.query = nn.Linear(n_embedding, head_size, bias=False)
#         self.value = nn.Linear(n_embedding, head_size, bias=False)
...
#         if self.pos_embed_method == 'rope':
#             # Reformer相对位置编码
#             if Head.freqs_cis is None:
#                 Head.freqs_cis = precompute_freqs_cis(dim=key.shape[-1], seq_len=T)
#             xq, xk = apply_rotary_emb(key, query, Head.freqs_cis)
#             # key*value/(d**-0.5)
#             query, key = xq, xk
#         wei = key @ query.transpose(-2, -1) * (key.shape[-1] ** -0.5)
#         # 掩藏操作，使注意力只能看到前面数据
...
#     """ 多头注意力 """
#
#     def __init__(self, num_heads, head_size, n_embedding=360, pos_embed_method="rope"):
#         super().__init__()
#         # 多头注意力由多个注意力叠加
#         self.heads = nn.ModuleList([Head(head_size, pos_embed_method=pos_embed_method) for _ in range(num_heads)])
#         self.linear = nn.Linear(n_embedding, n_embedding)
#         self.dropout = nn.Dropout(0.2)
#
#     def forward(self, x):
...
#
# class BlockOnlyPERO(nn.Module):
#     def __init__(self, n_emb, n_head, dropout=0.2):
#         super().__init__()
#         head_size = n_emb // n_head
